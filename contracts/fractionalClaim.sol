...
contract FractionalClaim {
    ...
    function claim(
        address _token,
        uint256 _amount
    ) public payable correctToken(_token) {
        ERC20Burnable(_token).transferFrom(
            msg.sender,
            address(this),
            _amount * 1000000000000000000
        ); //collect the token back, not working yet
        ERC20Burnable(_token).burn(_amount * 1000000000000000000); //claimed, so burn this token
        payable(msg.sender).transfer(
            (_amount * 1000000000000000000).div(supply)
        ); //send the ETH to the claimant
